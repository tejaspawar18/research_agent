...
from shared.utils import ScyllaDBManager, RedisManager, initialize_schema
...
class PipelineOrchestrator:
    ...
    async def run_pipeline(
        ...
        try:
            ...
            self._current = None
            self._runs[run.run_id] = run
            db_manager.insert_pipeline_run(run.model_dump())
        ...
        try:
            async with httpx.AsyncClient(timeout=300.0) as client:
                ...
                for _ in range(120):
                    ...
                    status_resp = await client.get(f"{CRAWLER_URL}/crawl/status/{job['job_id']}")
                    status = status_resp.json()
                    if status["status"] in ["completed", "failed"]:
                        break
                
                return await self._load_from_queue("articles:crawled")
        except Exception as e:
        ...
        try:
            async with httpx.AsyncClient(timeout=120.0) as client:
                response = await client.post(
                    ...
                    json={"articles": [a.model_dump() for a in articles]}
                )
                response.raise_for_status()
    ...
    async def _llm_process(self, articles: List[Article]) -> List[Article]:
        ...
        try:
            async with httpx.AsyncClient(timeout=600.0) as client:
                ...
                for article, proc_result in zip(articles, result["results"]):
                    if proc_result.get("status") == "processed":
                        ...
                        article.evidence_level = proc_result.get("evidence_level")
    ...
    async def _notify(self, articles: List[Article]) -> int:
        ...
        by_area: Dict[str, List[Article]] = {}
        for article in articles:
            area = article.project_area or "general"
            if area not in by_area:
                by_area[area] = []
            by_area[area].append(article)
        
        notified = 0
        
        try:
            async with httpx.AsyncClient(timeout=60.0) as client:
                for area, area_articles in by_area.items():
                    channel = config.slack.channels.get(area, "#research-general")
                    
                    response = await client.post(
                        f"{NOTIFICATION_URL}/notify/digest",
                        json={
                            "articles": [a.model_dump() for a in area_articles[:10]],
                            "project_area": area,
                            "channel": channel,
                        }
                    )
